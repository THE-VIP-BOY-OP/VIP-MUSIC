...
async def join_group(client, message):
    ...
    if message.chat.username and not await app.get_chat_member(chat_id, app.id).is_admin():
        await userbot.join_chat(message.chat.username)
        return
    
    # Condition 2: Group username is present, bot is admin and Userbot is banned
    if message.chat.username and await app.get_chat_member(chat_id, app.id).is_admin():
    ...
    if not message.chat.username and not await app.get_chat_member(chat_id, app.id).is_admin():
    ...
    if not message.chat.username and await app.get_chat_member(chat_id, app.id).is_admin():
        userbot_member = await app.get_chat_member(chat_id, userbot.id)
        if userbot_member.status in [ChatMemberStatus.BANNED, ChatMemberStatus.RESTRICTED]:
            try:
                await app.unban_chat_member(chat_id, userbot.id)
