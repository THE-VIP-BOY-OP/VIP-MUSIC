...
HEROKU_API_KEY = os.getenv("HEROKU_API_KEY")  # Make sure to set this in your environment
...
async def host_app(client: Client, message: Message):
    ...
    if status == 201:
        await message.reply_text("App successfully created! Now setting environment variables...")
        status, result = set_heroku_config_vars(
            user_inputs["HEROKU_APP_NAME"], user_inputs, HEROKU_API_KEY
        )

        if status == 200:
            await message.reply_text("App successfully deployed with environment variables!")
        else:
